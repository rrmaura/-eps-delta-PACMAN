    ...
    for i in range(n_treatments):
        for j in range(n_treatments):
            ...
            if alpha[i] > alpha[j]:
                ...
                denominator = special.beta(alpha[i]-2,beta[i]-2)*special.beta(alpha[j]-2,beta[j]-2)*(alpha[j]-alpha[i])
                prob_thetas[i,j] = 1-(numerator/denominator)
...
def exact_sum(alpha, beta, epsilon):
...
counter_es = np.zeros(array_shape)
...
for sample in n_sample_range:
    for epsilon in epsilon_range:
        for n_treatments in n_treatments_range:
            ...
            epsilon_opt_thetas = np.where(np.logical_and(theta>=theta[best_theta]-epsilon, theta<=theta[best_theta]+epsilon))
            print("Epsilon optimal treatments are " + str(epsilon_opt_thetas))
            ...
            for i in trange(sims): 
                ...
                j = 1
                rounds = math.floor(math.log2(n_treatments))
                if j < rounds:      
                    ...
                    j = j + 1
                if np.in1d(remaining_treatments,epsilon_opt_thetas).any()==1:
        ...
        for n_treatments in n_treatments_range:
            ...
            for i in trange(sims): 
                ...
                if j < rounds:      
                    ...
                    bool_survived_treatments = exact_sum(alphas[remaining_treatments], 
                                                            betas[remaining_treatments], 
                                                            epsilon)
                ...
                if np.in1d(remaining_treatments,epsilon_opt_thetas).any()==1:
                    ...
                    pos2 = epsilon_range.index(epsilon)
                    pos3 = n_treatments_range.index(n_treatments)
                    counter_es[pos1,pos2,pos3] = counter_es[pos1,pos2,pos3] + 1
